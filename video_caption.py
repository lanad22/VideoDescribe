...
def generate_scene_caption(frames, scene_data, available_gap, previous_caption=None, max_new_tokens=2048,
                         total_pixels=20480 * 28 * 28, min_pixels=16 * 28 * 28):
    """Generate a caption and classify it as inline or extended based on duration"""
    ...
    context = "\n".join(context_parts)
    max_words = int(available_gap * 1.2)
    ...
    f"Ensure the description fits within {available_gap:.1f} seconds when spoken, targeting about {max_words} words.\n"
    )
    
    # Generate caption using the model
    if isinstance(frames, np.ndarray) and len(frames) <= 128:
        frames_tensor = torch.tensor(frames, dtype=torch.float32).to('cuda')
        messages = [
        ...
        inputs = processor(text=[text], videos=frames_tensor, padding=True, return_tensors="pt")
    else:
        messages = [
            {"role": "system", "content": "You are a helpful assistant."},
            {"role": "user", "content": [
...
def process_all_scenes(video_folder):
    ...
    for idx, scene_data in enumerate(scene_list, start=1):
        ...
        # Skip caption generation if gap is too small
        if largest_gap < 0.5:
            print("Gap too small (<0.5s), skipping caption generation")
